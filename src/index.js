...
@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}.rot.is-live .rot__t{animation-name:rotFadeReduced}.dot{animation-name:blinkReduced}}
</style>
</head>
<body>
...
const names=${JSON.stringify(names).replace(/</g, "\\u003c")};const rots=["rot2","rot3","rot4"].map(id=>document.getElementById(id)).filter(Boolean);if(rots.length&&names.length>1){const texts=rots.map(node=>node.querySelector(".rot__t"));let index=0;texts[0].addEventListener("animationiteration",()=>{index=(index+1)%names.length;texts.forEach(node=>{node.textContent=names[index]})});rots.forEach(node=>node.classList.add("is-live"))}
</script>
</body>
</html>`;
}
