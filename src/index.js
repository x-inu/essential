...
.quick-command{display:block;white-space:nowrap;overflow-x:auto}.quick-command .rot,.readout--rows .rot{min-width:0}.command-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:1rem;align-items:baseline}.command-row__code{min-width:0;white-space:nowrap;overflow-x:auto}
@media(max-width:420px){.nav a[href^="https"]{display:none}.nav{gap:.75rem}.command-row{gap:.5rem}}
...
<main>
<section class="hero"><div class="wrap hero__grid"><div><p class="eyebrow"><span class="eyebrow__k">源</span> Tool index</p><div class="live"><span class="dot" aria-hidden="true"></span><span class="micro">Serving from the edge</span></div><h1>${DOMAIN}</h1><p class="lede">A small, explicit allowlist from <span class="mono">${REPO}</span>. Every published tool is available through a short, memorable URL.</p><div class="spec">${specRow("Source", REPO)}${specRow("Commit", commit)}${specRow("Tools", String(files.length).padStart(2, "0"))}${specRow("Payload", fmtSize(total))}${specRow("Edge cache", `${CACHE_TTL / 60} min`)}</div></div><div><p class="label usage-label">Quick use</p><div class="readout"><span class="quick-command"><span class="prompt">$ </span><span class="hl">curl -fsSL https://${DOMAIN}/</span><span class="rot" id="rot"><span class="rot__t">${sample}</span></span><span class="hl"> | sh</span></span></div><p class="micro usage-note">Choose a tool, copy the command, and run it in a privileged shell.</p></div></div></section>
...
<section id="how"><div class="wrap two"><div><p class="eyebrow"><span class="eyebrow__k">道</span> Trust path</p><h2 class="stmt">The manifest is the boundary, not a directory listing.</h2></div><div class="notes"><div class="readout">GET /v/${commit}/<span class="rot" id="rot4"><span class="rot__t">${sample}</span></span>
...
const names=${JSON.stringify(names).replace(/</g, "\\u003c")};const rots=["rot","rot2","rot3","rot4"].map(id=>document.getElementById(id)).filter(Boolean);if(rots.length&&names.length>1){const texts=rots.map(node=>node.querySelector(".rot__t"));let index=0;texts[0].addEventListener("animationiteration",()=>{index=(index+1)%names.length;texts.forEach(node=>{node.textContent=names[index]})});rots.forEach(node=>node.classList.add("is-live"))}
</script>
</body>
</html>`;
}
